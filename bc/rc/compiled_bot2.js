...
(function (bc19) {
    var Pilgrim = (function (_super) {
        ...
        Pilgrim.turn = function (myRobot) {
            ...
            if (Pilgrim.scouting) {
                if (Pilgrim.dangerEnemy == null && Pilgrim.attackTarget != null) {
                    if (bc19.Util.mr.getRobot(bc19.Util.mr.getVisibleRobotMap()[Pilgrim.attackTarget.y][Pilgrim.attackTarget.x]).unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Preacher].number()) {
                        var value = void 0;
                        var mx = 0;
                        for (var index123 = 0; index123 < Pilgrim.seenFriend.length; index123++) {
                            var po = Pilgrim.seenFriend[index123];
                            {
            ...
            if (bc19.Util.me.turn < 1000) {
                ...
                if (bc19.Util.me.karbonite === 20 || bc19.Util.me.fuel === 100) {
                    if ((bc19.Util.dest == null || Pilgrim.recalc % 4 === 3) && Pilgrim.lastTryc + 5 <= bc19.Util.me.turn) {
                        Pilgrim.recalc = 0;
                        bc19.Util.mainPath = bc19.Nav.backResourceToCC();
                        Pilgrim.lastTryc = bc19.Util.me.turn;
                    }
                    if (bc19.Util.dest != null && bc19.Util.dest !== bc19.Util.myp) {
                        bc19.Util.mr.log("Pere");
                        var po = (function (s) { return s[s.length - 1]; })(bc19.Util.mainPath);
                        if (bc19.Util.mr.getVisibleRobotMap()[po.y][po.x] === 0) {
                            /* pop */ bc19.Util.mainPath.pop();
                            Pilgrim.savePo = po;
                            Pilgrim.recalc = 0;
                            bc19.Util.mr.log("move(r) a pilgram. x: " + po.x + " y: " + po.y);
                            return bc19.Util.mr.move(bc19.Util.myp.dxTo(po), bc19.Util.myp.dyTo(po));
                        }
                        else {
                            ++Pilgrim.recalc;
        ...
        /*private*/ Pilgrim.justRunAway = function () {
            ...
            {
                var array125 = function () { var result = []; for (var val in bc19.Direction4) {
                    if (!isNaN(val)) {
                        result.push(parseInt(val, 10));
                    }
                } return result; }();
                for (var index124 = 0; index124 < array125.length; index124++) {
                    var dir = array125[index124];
                    {
                        ...
                        if (po != null && po.passable && bc19.Util.mr.getVisibleRobotMap()[po.y][po.x] === 0 && po.dist2(Pilgrim.dangerEnemy) > 16) {
                            Pilgrim.runAway = po;
                            break;
                        }
                    }
        ...
        /*private*/ Pilgrim.checkFight = function () {
            ...
            var poo = null;
            {
                var array127 = bc19.Util.mr.getVisibleRobots();
                for (var index126 = 0; index126 < array127.length; index126++) {
                    var r = array127[index126];
                    {
        ...
        /*private*/ Pilgrim.canGive = function () {
            {
                var array129 = function () { var result = []; for (var val in bc19.Direction) {
                    if (!isNaN(val)) {
                        result.push(parseInt(val, 10));
                    }
                } return result; }();
        ...
        /*private*/ Pilgrim.canMove = function () {
            ...
            var ddir = null;
            {
                var array131 = function () { var result = []; for (var val in bc19.Direction) {
                    if (!isNaN(val)) {
                        result.push(parseInt(val, 10));
                    }
                } return result; }();
...
(function (bc19) {
    var Castle = (function (_super) {
        ...
        Castle.turn = function (myRobot) {
            bc19.Util.setupt(myRobot);
            if (bc19.Util.me.turn < 12) {
                var dir = Castle.canProduceUnit();
                if (dir != null) {
                    return Castle.build3Pre1Pil(dir);
                }
            }
            return null;
        };
        ...
        /*private*/ Castle.canProduceUnit = function () {
            if (bc19.Util.mr.karbonite < 10 || bc19.Util.mr.fuel < 50)
                return null;
...
(function (bc19) {
    var Nav = (function (_super) {
                    ...
                    for (var index136 = 0; index136 < array137.length; index136++) {
                        var dir = array137[index136];
                        {
                            ...
                            if (pi != null && !marked[pi.x][pi.y] && bc19.Util.p[pi.x][pi.y].passable && (bc19.Util.myp !== po || bc19.Util.mr.getVisibleRobotMap()[pi.y][pi.x] <= 0)) {
                                if ((pi.karbonite || pi.fuel) && bc19.Util.mr.getVisibleRobotMap()[pi.y][pi.x] <= 0) {
                                    bc19.Util.dest = pi;
                                    ppo = po;
                                    /* push */ (path.push(pi) > 0);
                                    ...
                                    ;
                                    bc19.Util.mr.log("dest(t) seted. pathsize: " + path.length);
                                    return path;
                                }
                                /* add */ (que.push(pi) > 0);
                ...
                po = (function (a) { return a.length == 0 ? null : a.shift(); })(que);
                {
                    ...
                    for (var index138 = 0; index138 < array139.length; index138++) {
                        var dir = array139[index138];
                        {
                            pi = po.dir4To(dir);
                            if (pi != null && !marked[pi.x][pi.y] && bc19.Util.p[pi.x][pi.y].passable && (bc19.Util.myp !== po || bc19.Util.mr.getVisibleRobotMap()[pi.y][pi.x] <= 0)) {
                                if (bc19.Util.mr.getVisibleRobotMap()[pi.y][pi.x] > 0 && (bc19.Util.mr.getRobot(bc19.Util.mr.getVisibleRobotMap()[pi.y][pi.x]).unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Castle].number() || bc19.Util.mr.getRobot(bc19.Util.mr.getVisibleRobotMap()[pi.y][pi.x]).unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Church].number())) {
            ...
            ;
            {
                ...
                for (var index140 = 0; index140 < array141.length; index140++) {
                    var r = array141[index140];
                    {
                        if (r.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Castle].number()) {
                            if (vertical) {
                                bc19.Util.p[r.x][bc19.Util.size - r.y - 1].ecastle = true;
                                Nav.agoal = bc19.Util.p[r.x][bc19.Util.size - r.y - 1];
                            }
                            if (horizontal) {
                                bc19.Util.p[bc19.Util.size - r.x - 1][r.y].ecastle = true;
        ...
        Nav.AStar = function () {
            ...
            if (bc19.Util.me.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Pilgrim].number())
                return Nav.AStar4(Nav.agoal);
            else if (bc19.Util.me.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Crusader].number())
                return Nav.AStar9(Nav.agoal);
            else if (bc19.Util.me.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Prophet].number())
                return Nav.AStar4(Nav.agoal);
            else if (bc19.Util.me.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Preacher].number())
                return Nav.AStar4(Nav.agoal);
            return null;
        };
        Nav.reAStar = function () {
            ...
            if (bc19.Util.me.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Crusader].number())
                return Nav.AStar9(Nav.agoal);
            else if (bc19.Util.me.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Prophet].number())
                return Nav.AStar4(Nav.agoal);
            else if (bc19.Util.me.unit === bc19.UnitType["_$wrappers"][bc19.UnitType.Preacher].number())
        ...
        Nav.AStar9 = function (goal) {
            ...
            /* put */ (function (m, k, v) { if (m.entries == null)
                ...
                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                    m.entries[i].value = v;
                    return;
                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(parent, bc19.Util.myp, null);
            var g = ({});
            /* put */ (function (m, k, v) { if (m.entries == null)
                m.entries = []; for (var i = 0; i < m.entries.length; i++)
                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                    m.entries[i].value = v;
                    return;
                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(g, bc19.Util.myp, 0);
            var fm = ({});
            ...
            /* put */ (function (m, k, v) { if (m.entries == null)
                ...
                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                    m.entries[i].value = v;
                    return;
                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(f, bc19.Util.myp, value);
            hp = ([]);
            /* add */ (function (s, e) { if (s.indexOf(e) == -1) {
                s.push(e);
                return true;
            ...
            /* put */ (function (m, k, v) { if (m.entries == null)
                ...
                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                    m.entries[i].value = v;
                    return;
                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(fm, value, hp);
            fq.add(value);
            var fn = 0;
            var cry = 0;
            var ccry = 0;
            while ((!(openSet.length == 0))) {
                fn = fq.poll();
                hp = (function (m, k) { if (m.entries == null)
                    m.entries = []; for (var i = 0; i < m.entries.length; i++)
                    if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                ...
                po = (function (a) { var i = 0; return { next: function () { return i < a.length ? a[i++] : null; }, hasNext: function () { return i < a.length; } }; })(hp).next();
                if (!(openSet.indexOf((po)) >= 0)) {
                    ++cry;
                    continue;
                }
                if (hp.length === 1)
                    ...
                    (function (a) { return a.splice(a.indexOf(po), 1); })(hp);
                if (po.ecastle) {
                    bc19.Util.dest = po;
                    var ppo = po;
                    while (((function (m, k) { if (m.entries == null)
                        ...
                        if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                            return m.entries[i].value;
                        } return null; })(parent, ppo) != null)) {
                        /* push */ (path.push(ppo) > 0);
                        ppo = (function (m, k) { if (m.entries == null)
                            m.entries = []; for (var i = 0; i < m.entries.length; i++)
                            if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                return m.entries[i].value;
                            } return null; })(parent, ppo);
                    }
                    ;
                ...
                /* remove */ (function (a) { return a.splice(a.indexOf(po), 1); })(openSet);
                /* add */ (function (s, e) { if (s.indexOf(e) == -1) {
                    s.push(e);
                    return true;
                }
                ...
                {
                    var array143 = function () { var result = []; for (var val in bc19.Direction9) {
                        if (!isNaN(val)) {
                            result.push(parseInt(val, 10));
                        }
                    } return result; }();
                    for (var index142 = 0; index142 < array143.length; index142++) {
                        var dir = array143[index142];
                        {
                            ++Nav.ins;
                            pi = po.dir9To(dir);
                            if (pi == null || (closedSet.indexOf((pi)) >= 0) || !pi.passable || (bc19.Util.myp === po && bc19.Util.mr.getVisibleRobotMap()[pi.y][pi.x] !== 0))
                                continue;
                            gg = (function (m, k) { if (m.entries == null)
                                m.entries = []; for (var i = 0; i < m.entries.length; i++)
                                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                    return m.entries[i].value;
                                } return null; })(g, po) + 1;
                            if (!(openSet.indexOf((pi)) >= 0))
                            ...
                            else if (gg >= (function (m, k) { if (m.entries == null)
                                m.entries = []; for (var i = 0; i < m.entries.length; i++)
                                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                    return m.entries[i].value;
                                } return null; })(g, pi))
                            ...
                            /* put */ (function (m, k, v) { if (m.entries == null)
                                ...
                                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                    m.entries[i].value = v;
                                    return;
                                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(parent, pi, po);
                            /* put */ (function (m, k, v) { if (m.entries == null)
                                m.entries = []; for (var i = 0; i < m.entries.length; i++)
                                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                    m.entries[i].value = v;
                                    return;
                                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(g, pi, gg);
                            ...
                            if ((function (m, k) { if (m.entries == null)
                                ...
                                (function (a) { return a.splice(a.indexOf(pi), 1); })(/* get */ (function (m, k) { if (m.entries == null)
                                    m.entries = []; for (var i = 0; i < m.entries.length; i++)
                                    if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                        return m.entries[i].value;
                                    } return null; })(fm, /* get */ (function (m, k) { if (m.entries == null)
                                    m.entries = []; for (var i = 0; i < m.entries.length; i++)
                                    if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                        return m.entries[i].value;
                                    } return null; })(f, pi)));
                            /* put */ (function (m, k, v) { if (m.entries == null)
                                ...
                                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                    m.entries[i].value = v;
                                    return;
                                } m.entries.push({ key: k, value: v, getKey: function () { return this.key; }, getValue: function () { return this.value; } }); })(f, pi, value);
                            if ((function (m, k) { if (m.entries == null)
                                m.entries = []; for (var i = 0; i < m.entries.length; i++)
                                if (m.entries[i].key.equals != null && m.entries[i].key.equals(k) || m.entries[i].key === k) {
                                    return true;
                                } return false; })(fm, value))
                                (function (s, e) { if (s.indexOf(e) == -1) {
                                    s.push(e);
                                    return true;
                                }
        ...
        Nav.AStar4 = function (goal) {
            ...
            while ((!(openSet.length == 0))) {
                if (fq.size() === 0) {
                    bc19.Util.mr.log("BAD BUG 01 : " + openSet.length + " - " + closedSet.length + " - " + Nav.ins + " - " + cry + " + " + (function (a) { var i = 0; return { next: function () { return i < a.length ? a[i++] : null; }, hasNext: function () { return i < a.length; } }; })(openSet).next());
                    break;
                }
                Nav.flag = 1;
                fn = fq.poll();
                ...
                Nav.flag = 12;
                if (hp.length === 0) {
                    bc19.Util.mr.log("BAD BUG 02 : " + openSet.length + " - " + closedSet.length + " - " + Nav.ins + " - " + cry + " + " + (function (a) { var i = 0; return { next: function () { return i < a.length ? a[i++] : null; }, hasNext: function () { return i < a.length; } }; })(openSet).next() + " fn: " + fn);
                    break;
                }
                po = (function (a) { var i = 0; return { next: function () { return i < a.length ? a[i++] : null; }, hasNext: function () { return i < a.length; } }; })(hp).next();
                Nav.flag = 2;
        ...
        /*private*/ Nav.h = function (pi, goal) {
            return (Math.ceil((goal.dist2(pi) / Nav.speed | 0)) | 0);
        };
        return Nav;
    }(bc19.Util));
    Nav.agoal = null;
...
(function (bc19) {
    var Preacher = (function (_super) {
        ...
        /*private*/ Preacher.checkSignal = function () {
                ...
                for (var index146 = 0; index146 < array147.length; index146++) {
                    var r = array147[index146];
                    {
                        if (!bc19.Util.mr.isRadioing(r) || (bc19.Util.mr.isVisible(r) && bc19.Util.mr.getRobot(r.id).team !== bc19.Team["_$wrappers"][bc19.Util.teami].number()))
                            continue;
                        var s = r.signal;
                        var st = (s / (bc19.Util.size * bc19.Util.size) | 0);
                        if (st === 5) {
        ...
        /*private*/ Preacher.setAttackLand = function () {
            ...
            var poo = null;
            {
                var array149 = function () { var result = []; for (var val in bc19.Direction) {
                    if (!isNaN(val)) {
                        result.push(parseInt(val, 10));
                    }
                } return result; }();
                for (var index148 = 0; index148 < array149.length; index148++) {
                    var dir = array149[index148];
                    {
                        ...
                        if (po == null || !po.passable || po.dist2(bc19.Util.myp) > 16)
                            continue;
                        w = 0;
                        f = 0;
                        e = 0;
                        {
                            var array151 = function () { var result = []; for (var val in bc19.Direction) {
                                if (!isNaN(val)) {
                                    result.push(parseInt(val, 10));
                                }
                            } return result; }();
                            for (var index150 = 0; index150 < array151.length; index150++) {
                                var dir2 = array151[index150];
                                {
                                    poo = po.dirTo(dir2);
                                    if (poo == null)
                                        continue;
        ...
        /*private*/ Preacher.checkFight = function () {
            ...
            var poo = null;
            {
                var array153 = bc19.Util.mr.getVisibleRobots();
                for (var index152 = 0; index152 < array153.length; index152++) {
                    var r = array153[index152];
                    {
