	...
	private static void calcEnemyCastle() {
		...
		for (Robot r : mr.getVisibleRobots()) {
			if (r.unit == UnitType.Castle.number()) {
				if (vertical && p[r.x][size - r.y - 1].ocastleNotDes) {
					p[r.x][size - r.y - 1].ocastle = true;
					agoal = p[r.x][size - r.y - 1];
				}
				if (horizontal && p[size - r.x - 1][r.y].ocastleNotDes) {
...
//	}
	...
	public static Stack<Point> defCastleWithPreacher() {
		...
		outerloop: while (!que.isEmpty()) {
			...
			for (Direction4 dir : Direction4.values()) {
				...
				pi = po.dir4To(dir);
				if (pi != null && !marked[pi.x][pi.y] && p[pi.x][pi.y].passable
						&& (myp != po || mr.getVisibleRobotMap()[pi.y][pi.x] <= 0) && pi.dist2(myCastle) <= 16) {
	...
	public static Point expandTarget() {
		// ArrayList<Point> myexp = new ArrayList<Point>();
		int mn = 1000000, mnn = 1000000;
		int value;
		Point gg = null;
	...
	public static Stack<Point> nextAssignDefPre() {
		...
		outerloop: while (!que.isEmpty()) {
			...
			for (Direction4 dir : Direction4.values()) {
				...
				pi = po.dir4To(dir);
				if (pi != null && !marked[pi.x][pi.y] && p[pi.x][pi.y].passable
						&& (myp != po || mr.getVisibleRobotMap()[pi.y][pi.x] <= 0) && pi.dist2(myp) <= 16
						&& (myp != po || dir.number() < 8)) {
		...
		outerloop: while (!que.isEmpty()) {
			...
			for (Direction4 dir : Direction4.values()) {
				...
				if (pi != null && !marked[pi.x][pi.y] && p[pi.x][pi.y].passable
					...
					parent[pi.x][pi.y] = po;
					if ((pi.x + pi.y) % 2 == 0 && mr.getVisibleRobotMap()[pi.y][pi.x] <= 0 && !pi.karbonite && !pi.fuel
							&& pi.dist2(myCC) > 2 && ((top && pi.y >= myCC.y) || (down && pi.y <= myCC.y)
									|| (left && pi.x >= myCC.x) || (right && pi.x <= myCC.x))) {
						// mr.log("sis2");
						dest = pi;
						ppo = dest;
						while (parent[ppo.x][ppo.y] != null) {// ppo != myp
