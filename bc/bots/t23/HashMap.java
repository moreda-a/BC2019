...
public class HashMap implements HashMapInterface {
	...
	private int size;
	private Stack<MapEntry> list;

	public HashMap() {
		clear();
		list = new Stack<MapEntry>();
	}

	public Stack<MapEntry> getList() {
		return list;
	}

	public MapEntry getFirst() {
...
//		}
	...
	public Integer add(Point key, Integer value) {
		...
		if (entry == null) {
			...
			if (entry == null) {
				...
				mea = new MapEntry(key, value, entry.getNext());
				entry = mea;
			}
		}
		if (result == null)
	...
	@Override
	public void clear() {
		this.table = (MapEntry[]) new MapEntry[STARTING_SIZE];
		this.size = 0;
	}
...
//	}
	...
	private void resize() {
		...
		this.table = (MapEntry[]) new MapEntry[temp.length * 2];
		// this.size = 0;
		for (int i = 0; i < temp.length; i++) {
			MapEntry entry = temp[i];
			if (entry != null && !entry.isRemoved()) {
