...
//		}
	...
	private static UnitType nextUnit() {
		if (me.turn < 5) {
			...
			if (mr.karbonite == 70 && pre < 1)
				return UnitType.Preacher;
		}
		// if (pil < (karbonP.size() + fuelP.size()) / (xid.size() + 1))
		if (seenPil < resGoalP.size() && (pil < 2 || mr.karbonite >= 60 && mr.fuel >= 250))
...
//		}
		...
		// if (me.turn >= 50)
		{
			if (seenPil >= resGoalP.size() && pili < 2
					&& mr.karbonite >= mr.SPECS.UNITS[UnitType.Pilgrim.number()].CONSTRUCTION_KARBONITE
					&& mr.fuel >= mr.SPECS.UNITS[UnitType.Pilgrim.number()].CONSTRUCTION_FUEL)// pil < (karbonP.size()
																								// + fuelP.size()) / (2
																								// * xid.size() + 2))
			{
				pili++;
