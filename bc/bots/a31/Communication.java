...
public class Communication extends Util {
	public static int lastcmessage = -1;
	...
	public static int mss = 0;
	public static int msq = 0;

	public static void turn() {
		lastcmessage = cmessage;
		cmessage = 0;
		smessage = 0;
		maxSignalRange = 0;
		maxSignalRangeM = 0;
...
//					}
			...
			if (decode(r.castle_talk, 2, 4) == 0) {// [XXXX0010]
				...
				if (decode(r.castle_talk, 7, 8) == 1) {
					Castle.pullNow = true;
					Castle.stopUsingRes = true;
				} // it is frist turn and important
...
//					+ (r.signal / (int) Math.pow(2, 4)) + " - " + p[decode(r.signal, 10, 16)][decode(r.signal, 4, 10)]);
			...
			if (Preacher.mode == 0 || Preacher.mode == 2)
				encode(castlei.size() * 16 + 1 * 4, 0, 6);
			else
...
//			+ (r.signal / (int) Math.pow(2, 4)) + " - " + p[decode(r.signal, 10, 16)][decode(r.signal, 4, 10)]);
			...
			// if (Preacher.mode != 2)
			if (Preacher.mode != 1) {
				Preacher.lastmode = Preacher.mode;
				Preacher.mode = 2;
				kkp = p[decode(r.signal, 10, 16)][decode(r.signal, 4, 10)];
			}
		}
	...
	public static void signalOutCastle() {
		...
		for (Robot r : seenOR) {
			Point po = p[r.x][r.y];
			if (po == null || r.unit == UnitType.Preacher.number())
				continue;
			v = true;
			for (Point pp : seenIPreP) {
				if (pp == null)
					continue;
				if (po.dist2(pp) <= 16) {
					v = false;
				...
				if (myp.dist2(pp) <= 16)
					msq = Math.max(msq, myp.dist2(pp));
			}
			if (v && seenIPre != 0) {
				smessage = po.x * (int) Math.pow(2, 10) + po.y * (int) Math.pow(2, 4) + 13;
				break;
			}
		}
		if (msq != 0 && v && seenIPre != 0 && smessage % 16 == 13) {
			return;
		} else {
			// msq = 0;
			...
			if (Castle.pullNow) {
				...
				if (maxSignalRangeExpand != 0) {
					...
					if (Castle.expTarget != null) {
						smessage = Castle.expTarget.x * (int) Math.pow(2, 10)
								+ Castle.expTarget.y * (int) Math.pow(2, 4) + 10;
							...
							if (maxSignalRange == 0) {
								...
								if (xid.size() == 2) {
									...
									for (Integer id : xid.keySet()) {
										if (t == 0) {
											smessage = xid.get(id) * (int) Math.pow(2, 10)
													+ yid.get(id) * (int) Math.pow(2, 4) + 11;
											// [XXXX,XXYY,YYYY,1101]
											break;
										}
										t--;
	...
	public static void signalOut() {
		if (msq != 0) {
			mr.signal(smessage, msq);
			mr.log("signal : " + smessage + " - " + (smessage % 16) + " - " + msq);
		} else {
			if (mss != 0) {
				mr.signal(smessage, mss);
				mr.log("signal : " + smessage + " - " + (smessage % 16) + " - " + mss);
			} else {
				if (maxSignalRangeExpand != 0) {
					mr.signal(smessage, maxSignalRangeExpand);
					mr.log("signal : " + smessage + " - " + (smessage % 16) + " - " + maxSignalRangeExpand);

				} else {
					if (maxSignalRangeM != 0 && Castle.wave != 0) {
						mr.signal(smessage, Math.max(maxSignalRangeM, maxSignalRange));
						mr.log("signal : " + smessage + " - " + (smessage % 16) + " - "
								+ Math.max(maxSignalRangeM, maxSignalRange));
					} else if (maxSignalRange != 0) {
						mr.signal(smessage, maxSignalRange);
						mr.log("signal : " + smessage + " - " + (smessage % 16) + " - " + maxSignalRange);
					}
				}
			}
		}
...
//					+ (r.signal / (int) Math.pow(2, 4)) + " - " + p[decode(r.signal, 10, 16)][decode(r.signal, 4, 10)]);
	...
	public static void signalInProphet() {
		// encode(castlei.size() * 16 + 1 * 4, 0, 6);
		// Prophet.stopCommand = false;
		// what about radio enemy?!!?
		for (Robot r : mr.getVisibleRobots()) {
			if (!mr.isRadioing(r) || (mr.isVisible(r) && mr.getRobot(r.id).team != teami.number()))
				continue;
			handleSignalInProphet(r);
			if (r.id != me.id)
				mr.log("sz :" + r.id + " - " + r.signal);
	...
	private static void handleSignalInProphet(Robot r) {
		int opcode = decode(r.signal, 0, 4);
		if (opcode == 5) {// [DDDD,DDDD,DDDD,0101] -> scout tell me to stop
			// Prophet.stopCommand = true;
			// stop command
		} else if (opcode == 6) {// [XXXX,XXYY,YYYY,0110]
//			mr.log("hrll : " + decode(r.signal, 10, 16) + " - " + decode(r.signal, 4, 10) + " - "
//					+ (r.signal / (int) Math.pow(2, 4)) + " - " + p[decode(r.signal, 10, 16)][decode(r.signal, 4, 10)]);
...
//			if (Prophet.mode == 0 || Prophet.mode == 2)
//				encode(castlei.size() * 16 + 1 * 4, 0, 6);
//			else
//				encode(castlei.size() * 16 + 2 * 4, 0, 6);
...
//			+ (r.signal / (int) Math.pow(2, 4)) + " - " + p[decode(r.signal, 10, 16)][decode(r.signal, 4, 10)]);
			...
			// if (Preacher.mode != 2)
//			if (Prophet.mode != 1) {
//				Prophet.lastmode = Preacher.mode;
//				Prophet.mode = 2;
//				kkp = p[decode(r.signal, 10, 16)][decode(r.signal, 4, 10)];
//			}
		}
