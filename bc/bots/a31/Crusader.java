...
public class Crusader extends Util {
	...
	public static Point firstDest = null;
	...
	public static Action turn(MyRobot myRobot) {
		...
		if (me.turn == 1) {
			...
			if (mr.karbonite == 85 && mr.fuel == 475)
				firstIdiot = true;
		}
		checkFight();
		// mr.log("idiot1");
		if (dangerEnemy == null && attackTarget != null) {
			...
			if ((dest == null && lastTry + 5 <= me.turn) || (me.turn > 6 && lastmyp.get(me.turn - 5).x == myp.x
				...
				if (mode == 0) {
					// if (madeByCastle)
					if (firstIdiot) {// firstDest == null
						mainPath = Nav.harasCru();
						if (firstDest == null && dest != null)
							firstDest = dest;
					} else
...
//		}
			if (dest != null && dest != myp) {
				...
				if (mode == 5 && myp.dist2(po) == 4) {
					mr.log("ss3");
					int xx = myp.x + myp.dxTo(po) / 2;
					int yy = myp.y + myp.dyTo(po) / 2;
					if (canReach(p[xx][yy])) {
						// recalc = 0;
						mr.log("move slowRush a cru. x: " + p[xx][yy].x + " y: " + p[xx][yy].y);
				...
				if (canReach(po)) {
					...
					savePo = po;
					// recalc = 0;
					mr.log("move(1) a cru. x: " + po.x + " y: " + po.y);
...
//				}
					if (mainPath.size() == 1) {
						...
						if (canReach(p[xx][yy])) {
							// recalc = 0;
							mr.log("move(2) a cru. x: " + p[xx][yy].x + " y: " + p[xx][yy].y);
							return mr.move(myp.dxTo(p[xx][yy]), myp.dyTo(p[xx][yy]));
						}
					} else {
						// mr.log("HERE2");
						...
						if (canReach(p[xx][yy])) {
							// recalc = 0;
							mr.log("move(3) a cru. x: " + p[xx][yy].x + " y: " + p[xx][yy].y);
							return mr.move(myp.dxTo(p[xx][yy]), myp.dyTo(p[xx][yy]));
						} else {
							// mr.log("HERE4");
							mainPath.push(po);
							...
							if (canReach(p[xx][yy])) {
								// recalc = 0;
								mr.log("move(4) a cru. x: " + p[xx][yy].x + " y: " + p[xx][yy].y);
								return mr.move(myp.dxTo(p[xx][yy]), myp.dyTo(p[xx][yy]));
							}
							// mr.log("HERE5");
						}
