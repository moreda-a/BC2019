...
public class Church extends Util {
	...
	public static boolean waitingForPil = false;
	public static boolean xdone = false;
	public static boolean ydone = false;
...
//		}
		...
		if (me.turn == 1) {
			Communication.cmessage = UnitType.Church.number() * 16 + 0 * 4 + 2;// [XXXX0010]
		} else if (me.turn > 1 && !ydone) {
			if (!xdone && Communication.lastcmessage != myp.x * 4 + 1)
				Communication.cmessage = myp.x * 4 + 1;// [XXXXXX01]
			if (Communication.lastcmessage == myp.x * 4 + 1) {
				xdone = true;
			}
			if (xdone && Communication.lastcmessage != myp.y * 4 + 1)
				Communication.cmessage = myp.y * 4 + 1;// [XXXXXX01]
			if (xdone && Communication.lastcmessage == myp.y * 4 + 1)
				ydone = true;
		}
		...
		if (needRes) {
			if (me.turn > 1) {
				...
				if (seenO.size() >= 2)
					Communication.cmessage = 3 * 4 + 2;
			} else if (me.turn == 1)
				Communication.cmessage += 32;
...
//		}
	...
	private static UnitType goForRes() {
		...
		if (seenIPilNear < resGoalP.size() && (pil < 2 || mr.karbonite >= 40 && mr.fuel >= 100) && pil < 10
			...
			return UnitType.Pilgrim;
		if (seenIPilNear < resGoalP.size() && (me.turn > 20 && mr.karbonite >= 10 && mr.fuel >= 50) && pil < 10
				&& seenO.size() == 0) // seenIpil
			waitingForPil = true;
		return null;
	}
	...
	private static UnitType readyForDef() {
		if (mr.karbonite >= 50 + 30 && mr.fuel >= 200 + 50 && seenIProNear < 3)
			return UnitType.Prophet;
		if (pro < seenO.size() && mr.karbonite >= 30 && mr.fuel >= 50)
			return UnitType.Prophet;
		if (pro < 3 && mr.karbonite >= 40 && mr.fuel >= 100 && me.turn > 20)
			return UnitType.Prophet;
