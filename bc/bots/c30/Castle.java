...
public class Castle extends Util {
	...
	public static Action turn(MyRobot myRobot) {
		...
		if (attackTarget != null
				&& (ut == null || (ut != UnitType.Preacher && (ut != UnitType.Prophet || myp.dist2(attackTarget) <= 16))
						|| cpu == null || mr.karbonite < mr.SPECS.UNITS[ut.number()].CONSTRUCTION_KARBONITE
...
//		}
	...
	private static void assign() {
		...
		if (ut == UnitType.Pilgrim && me.turn < 100) {
			...
			// mainPath = Nav.nextAssignDefPro();
			mainPath = Nav.nextAssignDefBackProX();
			if (mainPath != null && mainPath.size() != 0) {
				assignNode = mainPath.pop();
			}
		} else if (ut == UnitType.Preacher) {
...
//		}
	...
	private static UnitType goForDef() {
		...
		mr.log("ST: " + seenIPreNear + " - " + seenIProNear);
		if (seenO.size() != 0 && seenIPreNear + seenIProNear < seenO.size()) { // 1-0 1-1 2-1 3-1 3-2 4-2 5-2 5-3
			if (seenIPreNear <= 2 * seenIProNear && (me.health == 200 || pro != 0)) {
				...
				if (myp.dist2(attackTarget) <= 16 && mr.karbonite >= 30)
					return UnitType.Preacher;
				else
					return UnitType.Prophet;
...
//			}
	...
	private static UnitType readyForPush() {
		...
		if (me.turn >= 20) {// && me.turn < 150) {
			if (mr.karbonite >= 15 * (4 - (pre % rushSize)) && mr.fuel >= 25 * (4 - (pre % rushSize))
					&& mr.karbonite >= mr.SPECS.UNITS[UnitType.Preacher.number()].CONSTRUCTION_KARBONITE + 10
					&& mr.fuel >= mr.SPECS.UNITS[UnitType.Preacher.number()].CONSTRUCTION_FUEL + 10
...
//				* Math.max(estFighter, seenIPreNear) && seenIPreNear >= 4)) 
		{
			if (Communication.smessage % 16 == 11 || Communication.smessage % 16 == 9
					|| Communication.smessage % 16 == 8 || Communication.smessage % 16 == 7)
				wave = 0;
