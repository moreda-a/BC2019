...
//		}
	...
	private static void calcEnemyCastle() {
		...
		if (myCC == null)
			mr.log("bullshit?!!:D");
		if (revPoint(myCC).ocastle || !Prophet.madeByCastle || !Crusader.madeByCastle)
			agoal = revPoint(myCC);
		// felan benzaram harki bere be rev khodesh hum ? :D
		if (vertical && horizontal)
			mr.log("FuckVFuckH :((");
		// mr.log("ll: " + agoal + "ss: " + myCC);
...
//				}
	...
	public static Stack<Point> nextAssignDefBackProX() {
		// mr.log("nobugpls");
		Stack<Point> path = new Stack<Point>();
		Queue<Point> que = new LinkedList<Point>();
		Point[][] parent = new Point[size][size];
		...
		outerloop: while (!que.isEmpty()) {
			...
			for (Direction4 dir : Direction4.values()) {
				...
				pi = po.dir4To(dir);
				if (pi != null && !marked[pi.x][pi.y] && p[pi.x][pi.y].passable && (myp != po || dir.number() < 8)
						&& (myp != po || (mr.getVisibleRobotMap()[pi.y][pi.x] <= 0 && Prophet.isItSafeX(pi)))
						&& (mr.getVisibleRobotMap()[pi.y][pi.x] <= 0
								|| (mr.getRobot(mr.getVisibleRobotMap()[pi.y][pi.x]).unit != UnitType.Castle.number()
										&& mr.getRobot(mr.getVisibleRobotMap()[pi.y][pi.x]).unit != UnitType.Church
												.number()))
...
//		return gg;
	...
	public static Stack<Point> oneStepToTarget() {
		Stack<Point> path = AStarIsBack4p(kkp);
		if (path != null && path.size() != 0) {
			return path;
		}
		return null;
	...
	public static Stack<Point> slowAttEnemeyCastle() {
		mr.log("LastAStar:" + ins + " flag: " + flag);
		agoal = null;
		calcEnemyCastle();
		if (agoal == null) {
