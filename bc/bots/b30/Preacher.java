...
//		}
		...
		if (me.turn == 1) {
			for (Robot r : mr.getVisibleRobots()) {
				...
				if (mr.isVisible(r) && r.team == teami.number() && r.unit == UnitType.Church.number()) {
					if (myp.dist2(p[r.x][r.y]) <= 2) {
						myCC = p[r.x][r.y];
					}
				}
		...
		if (attackTarget != null) {
			...
			if (mode == 2) {
				subPath = Nav.oneStepToTarget();
				if (subPath != null && subPath.size() != 0) {
					Point po = subPath.peek();
					if (canReach(po)) {
						mainPath.pop();
						savePo = po;
						// recalc = 0;
						mr.log("move(z) a preacher. x: " + po.x + " y: " + po.y);
